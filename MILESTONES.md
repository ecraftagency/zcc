...
- **M18 (dự kiến) — chiến dịch correctness 100%** (Vu chốt 4 khía 2026-08-17; bổ sung 2026-08-18: sau khi pass hết suite hiện hành, replenish theo 2 trục — (a) độ phủ định lý cao nhất có thể, (b) suite công nghiệp bổ sung).
  - **CẬP NHẬT 2026-08-21 — Csmith/yarpgen TÁI KÍCH HOẠT (trục correctness IR-era):** điều kiện nhập của "HOÃN-CÓ-ĐIỀU-KIỆN" (tool ở vai máy-sinh/trọng tài NGOÀI repo như gcc, cài trong box không vendor không vào build path) nay THỎA — csmith 2.3.0 + yarpgen 2.0 cài apt/build trong image `zcc-box:fuzz`, oracle gcc, differential 3-way (gcc vs zcc-noopt vs zcc `ZCC_OPT=1`). Kết quả đợt đầu: **csmith 300 (seed1-300) → 300 PARITY / 0 DIVERGE** sau khi trị 1 lowering bug (orphan-temp dead-code, commit `5c6a65d`); yarpgen + csmith seed 301+ đang mở rộng. Đây là chứng nghiệm THỰC TIỄN đắt (tầng dưới sci-gate) đúng doctrine hai-tầng: case đơn-file deterministic, seek-được, thỏa luật tốc-độ.
  - **CHỐT 2026-08-21 — CHIẾN DỊCH SEAL-IR HOÀN TẤT (nhánh `seal-ir-10k` → main):** IR→ops→asm nay là **đường DUY NHẤT**. 13/13 exotic node đã seal thành typed `Inst` (Block/stmt-expr, Alloca/VLA, CallX composite+SRet, Sync/atomic, Asm/inline, addr-of-exotic…); `Inst::Opaque` + toàn bộ AST-walk backend (`emit`/`expr`/`call`/`stmt`/`addr`, ~518 LOC) đã XOÁ; **không còn cờ `--ir`** (backend không có công tắc chọn đường — không compiler thật để cờ đó; `--ir` đã bỏ khỏi driver + mọi script). Kiến trúc đạt đúng đích Vu: `ast → ir,ops → s`. LOC src: 11344 → **11023**. Gate đóng sổ (default=IR): torture 1377/0 · musl-box PASS (1350 obj/6.5MB libc.a, SMOKE byte-identical) · cargo 29/29 · 0 warning · **csmith 300 PARITY / 0 DIVERGE** (evidence 162MB ELF thật ⟹ không no-op). Bằng chứng cơ học Opaque chết trước khi xoá: probe 0 bridge-hit / 3748 file thật.
