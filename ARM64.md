...
`sqlite3.c` unless the row says otherwise. **Ratios are against `gcc -O1` up to
2026-08-29 and against `gcc -O2` after it** (`MEASURED M49`), and against an Apple
M1 Pro up to that date and a Graviton4 after it (`M46`) — a row's number is only
readable with both. The two referees are far apart: the same compiler reads 1.06
against -O1 and 1.31 against -O2 on the same 96 programs.
...
| **The inline-copy bound was re-derived on the TIME axis** (`MEASURED M40`, `INLINE_COPY_MAX` 32 → 128) | `v3_struct_copy` **1.574× → 1.099×** (2,227 → 1,554 µs), dynamic instructions 2.410 → **1.420**; sqlite +131 static instructions, +0.075% | `M14` swept the bound against sqlite's STATIC count and found a clean minimum at 32, which it is — a call is four instructions whatever the length. What the static count cannot see is what the call then EXECUTES: measured against always-inline at ten sizes, `bl memcpy` costs a near-constant **12 extra dynamic instructions and never wins, out to 512 bytes**. The program was calling `memcpy` four times per iteration to assign a 64- and a 96-byte struct. **A constant derived on the size axis is not a constant on the time axis**, and `M38`'s corr(INSN, EXEC) = 0.196 is the general statement of why |
| **Granlund–Montgomery division by a constant** (`MEASURED M47`, `hir/pass/divmagic.rs`) | `a2_udiv_mod` **5.52× → 1.19×**, `a3_sdiv_mod` 2.10 → **0.51**, `ab2_format` 1.41 → **0.88**; suite EXEC on Graviton4 **1.1716 → 1.0902**, INSN 1.0751 → 1.1091 | The row `M25` built, proved, measured and DELETED — on an M1 Pro, where `udiv` is cheap. On Neoverse V2 it is 4.98 dependent adds, and the same theorem is worth 7% of the whole suite. `M25`'s second precondition (reach gcc's five instructions) was not one either: with the magic constant rebuilt every iteration the row still goes 4.455 → 1.111. **A row deleted on one core's evidence is a row deleted on one core's evidence** |
