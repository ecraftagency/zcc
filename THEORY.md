...
| SSA construction (Braun 2013, on-the-fly, no dominance frontier) | `⟦f⟧ = ⟦to_ssa(f)⟧` (semantics-preserving promotion of non-address-taken locals; a promoted `float`(size 4) `Load` becomes a narrowing self-`Cast`, since `Store∘Load` on a size-4 float cell = round-to-f32, not identity — C99 6.3.1.5) | **DONE (Stage 2)** — `opt::to_ssa`; proven by `to_ssa_semantics_preserved` (312 exprs × equiv), `to_ssa_diamond_and_loop`, `to_ssa_respects_address_taken`, `to_ssa_gate_has_teeth` |
