...
> Current status: the SSA pipeline `opt::optimize_ssa` (`src/opt/`) is the default
> optimizer — every pass is PROVEN at the IR→IR level *before* the corpus ever runs:
> the ⟦·⟧-rewrites by `ir::tests::equiv` (the commuting square ⟦A⟧≡⟦P(A)⟧ over the
> small-domain-exhaustive battery + boundaries) plus `verify`; regalloc (liveness
> dataflow-fixpoint → interference → Chaitin coloring → `verify_coloring`) by the
> INTERFERENCE INVARIANT (rename-bisimulation); the backend `.s` peepholes by machine
> translation-validation (opt-parity 0-DIVERGE). The per-theorem catalog is the pass
> ladder below; run behind the `ZCC_OPT` flag on the default IR path.
...
| Operational semantics (small/big-step) | interp realizes ⟦·⟧ (Σ=⟨ρ,μ⟩); executable theorem, `SEMANTICS.md` §5. Bounded by a depth guard (host-stack safety) AND a GLOBAL step budget shared across the whole recursion — the depth guard alone bounds only a deep *spine*, never a shallow-but-EXPONENTIAL call tree (fib(255) bottoms out at depth ~253 yet is ~10⁵³ nodes); an input past the budget is "outside the modeled space" → `equiv` SKIP, exactly like UB/too-deep. (Found via the inline self-recursion proof: a per-frame budget hung on the `equiv` battery.) |
...
| MULTIPLY-ADD FUSION (Tier-1 #3 — `madd`) | pattern `Add(Mul(x,y), c)` (commutative) where the Mul feeds ONLY that Add (`use_count==1`), both integer and SAME width ⟹ one `madd xD,xX,xY,xC` = c + x·y, DELETING the `mul`. `⟦·⟧` preserved by a ℤ/2ⁿ argument: the original truncates the product to n bits before adding, `madd` keeps 64 bits, but the FINAL `ext_r` to width n makes them equal — `(c+trunc_n(x·y)) ≡ (c+x·y) (mod 2ⁿ)`, and `ext_r` observes only those low n bits (signedness is irrelevant to `mul`'s low bits). Machine translation-validation | **DONE (Tier-1 #3), default-ON.** `arm64_elf::try_fuse_madd`. Measured (box): matmul inner product `mul;add` → one `madd x19,x21,x22,x19`; **bench geomean 0.78×→0.74×** (loops 0.44→0.37×, matmul 1.25→1.23×). Gated: opt-parity 1552/0 + torture 1378/0. `msub` (`Sub(c,Mul)`) not yet fused — a symmetric extension |
| ADDRESSING-MODE FOLD (Tier-1 #2 — BURS / maximal munch on the address tree) | tree-pattern matching: `Load(Add(base, index))` where the Add feeds ONLY that Load (function-wide `use_count==1`) and both operands are register-resident ⟹ ONE register-offset load `ldr* xD,[xBase,xIndex]`, DELETING the separate `add`. `[Xn,Xm]` adds the full 64-bit Xm ⟹ the same effective address; the add is dead (single-use) ⟹ deleting it preserves `⟦·⟧`. Gated: ct is 8-byte (an address is never narrowed), the load is simple-GP, and `reg_uses` counts BOTH bracket registers as reads (so the later move-peephole cannot mistake the index for dead — the DCE-lesson class of bug, pre-empted). Machine translation-validation | **DONE (Tier-1 #2), default-ON.** `arm64_elf::{try_fuse_addr, load_idx}` + the `Inst::Load` compute-into-home path (`load_gp`), driven by a function-wide temp READ count from the authoritative `opt::each_use` visitor (single source of truth ⟹ no missed use). Measured (box): matmul inner loop drops both index `add`s (`add xT,base,idx; ldrsw xV,[xT]` → `ldrsw xV,[base,idx]`); **bench geomean 0.81×→0.78×** (matmul 1.38→1.25×). Gated: opt-parity 1552/0 + torture 1378/0. Struct-field `[base,#imm]` and `lsl`-scaled forms not yet folded (index materialized as a shared `mul` ⟹ not single-use) — a later extension |
...
| ALIAS ANALYSIS (B1 — the memory oracle; QBE `alias.c` re-derived, NOT transliterated) | Aliasing is a TOTAL DECIDABLE relation over a 4-point base lattice `{Loc(off), Sym(k,i), Con, Unk(t)}` + an integer offset — no points-to graph, no fixpoint, ONE RPO pass. Each address value gets `(base, off)`: `Lea Local`→Loc, `Lea Global/Str`→Sym, `Imm`→Con, and pointer±CONST propagates the base (a ring identity on the address — `off ∈ ℤ/2⁶⁴`, so base-offset arithmetic is `wrapping_add`/`wrapping_sub`, never checked/trapping); anything else→Unk. `escaped` = the stack slots whose address LEAKED (call arg / stored-through / returned / mixed into an Unk pointer); a non-escaped Loc is PROVABLY disjoint from every Unk pointer (QBE's key move). Relation `alias((p,sp),(q,sq))∈{No,Must,May}`: two different slots→No; same base→overlap?Must:No; one Unk vs a provably-local slot→No, vs anything else→May; disjoint kinds→No; where overlap = `ap.off<aq.off+sq ∧ aq.off<ap.off+sp`. **SOUNDNESS (the CbC obligation): the oracle NEVER answers No/Must when two accesses can actually alias — May is always safe.** An ANALYSIS, not a transform ⟹ no code, no improvement obligation; its correctness IS its proof obligation (Law 3). Escape (`AEsc`) falls out FREE (Tier-4 #21 escape analysis) | **DONE (B1), analysis-only (consumed by B2).** `opt::{alias_info, AliasInfo, ABase, AliasR}` (side table, computed on demand — a backend/opt concern, IrFunc stays the frontend/backend boundary). Proven by `alias_verdicts` (every relation arm: same-slot overlap / disjoint offset / distinct slots / sym=/sym≠ / local-vs-unknown escaped & not / unknown-vs-unknown) + `alias_soundness` (a battery of hand-built access pairs whose TRUE aliasing is known — the oracle never lies). **Realization fix (2026-08-22, yarpgen s0186/s0231):** base-offset used checked `i64` `-`/`+` → debug-mode overflow panic on large 3D-array byte offsets; the "ring identity" theorem was already stated, the CODE just didn't honor it — switched to `wrapping_sub`/`wrapping_add` (mem.rs), a pure Side-I fix. Both cases now PARITY (gcc==zcc-O0==zcc-opt); ABI gate 312×4/0 |
| LOAD ELIMINATION / STORE→LOAD FORWARDING (B2 — gated by the B1 oracle; QBE `load.c`) | Within a straight-line block a `Load` from `a` (width sa) whose value is already in memory — a preceding `Store` wrote a MUST-alias address of the SAME width, with NO intervening MAY-alias store — equals that stored value ⟹ forward it (`Load`→`Copy`). A load whose value came from an earlier same-width must-alias load is likewise redundant. The oracle also lets an available value SURVIVE a store it provably does-not-alias (block-local `cse` conservatively kills EVERY load at ANY store; B2 keeps the NoAlias ones). BLOCK-LOCAL ⟹ "preceding" = "dominating on the sole path", no dominance machinery. Floats EXCLUDED (a `float`(4) store rounds to f32; `Copy` does not — forwarding would skip the rounding, C99 6.3.1.5). Any opaque memory writer (call/memcpy/zero/exotic) clears the available set (allowlist of pure ops; a future exotic kills by default). `⟦f⟧=⟦load-elim(f)⟧` | **DONE (B2), default-ON.** `opt::{load_elim, pure_mem}` in the `optimize_ssa` fixpoint after `cse`. **Correctness** — the commuting square via `equiv`: interp's per-frame `mem` machine-validates a forwarded LOCAL load; global/unknown loads are unmodeled → `equiv` SKIP, correct by the B1 soundness theorem + the box torture (the same backstop that caught the 11 inline regressions `equiv` was blind to). **Improvement** — the emitted `.s` has strictly FEWER `ldr` (each forwarded load → a register copy the peephole folds); statically visible, no race. Proven: `load_elim_forwards_store`, `load_elim_survives_nonalias_store`, `load_elim_semantics_preserved` (equiv). Gated: torture 1378/0 + bench |
| FUNCTION INLINING (Tier-1 #5 — β-reduction across the call graph; the ONLY call-overhead lever) | `⟦Call f(args)⟧ = ⟦body_f with params bound to args, each Ret v → (dst := v; goto cont)⟧`. INTERPROCEDURAL ⟹ runs on straight-lowered IR BEFORE `to_ssa` (reads the whole `funcs` set), the per-function SSA passes then β-substitute + DCE the spliced copies. **Frame model (the one Side-II fact):** a local at offset `off` lives at index `frame−off` / address `x29−off` — ONE offset law shared by `ir.rs::interp` and `arm64_elf::lea_local`. Merge the callee frame WITHOUT disturbing the caller's own offsets by APPENDING below: callee local `off_k` → `off_k + frame_base`, merged `frame += callee.frame` ⟹ caller locals `[0,F_c]` and clone locals `[F_c,F_c+F_k)` are DISJOINT under both index laws. Depth-1 bound: only call sites in the caller's ORIGINAL blocks expand (against a pre-mutation snapshot) ⟹ self-recursion is a 1-level unroll (finite), non-recursion substituted once. Cost model: leaf ≤16 / self ≤40 insts. **Restrictions (correctness envelope):** scalar-only params + scalar/void return (aggregate byval/sret is copied by Memcpy the scalar `Store`-seed cannot model — else struct-by-value miscompiles, gcc 20000706-1/pr20621-1); variadic/VLA CALLERS excluded (the frame-append lands in their AAPCS64 reg-save / dynamic-SP region). Commuting-square translation-validation (`equiv` recurses through the residual `Call(Sym)`) | **DONE (Tier-1 #5), default-ON.** `opt::{inline, splice, inline_ok, scalar_ty}`; backend reserves the SP from the IR (inline-grown) `funcs[fi].frame` as the single frame source (`g.fframe`, emit_params va-region + `lea_local` + `sp_adjust`). Proven: `inline_leaf_commutes`, `inline_void_commutes`, `inline_self_recursion_depth1` (⟦fib⟧=⟦inline fib⟧). Measured (box): **bench geomean 0.74×→0.69× vs gcc-O0, fib 1.38→1.05× (near par)**. Gated: torture **1378/0** — the box caught 11 regressions the `equiv` model was BLIND to (4 struct-by-value → scalar-only gate; 7 SEGFAULT → the AST-vs-IR frame-source split), each a Side-I/II fix, not a test edit. Opens IPA-CP (#18) |
| IF-CONVERSION → csel (B4 — control→data; the branchy-code lever, consumes the B1 oracle) | A DIAMOND `h:…;Br(c,T,E)  T:s_T;Jmp M  E:s_E;Jmp M  M:merge(d)…` whose two arms `s_T,s_E` are PURE + NON-FAULTING equals executing BOTH arms unconditionally (speculation) then selecting per `c`: `⟦diamond⟧ = ⟦h;s_T;s_E; Select(d,c,vT,vE); M∖merge⟧`. Justification (Side-I): a pure non-faulting instruction has NO observable effect when its result is unused, so running the not-taken arm is invisible; the merge (which picks the value of the edge actually taken) becomes exactly `c ? vT : vE`. **NON-FAULTING is the one subtle premise** — `Bin(Div\|Rem)` on an INTEGER traps on /0 (excluded); `Load(a)` faults on a bad address ⟹ speculatable ONLY where the B1 oracle proves `a` MAPPED (a stack slot `Loc` or a symbol `Sym` — `fault_free`); Store/Call/Memcpy/… have effects (never an arm). **This is where B4 consumes B1 (the ★★★★★ enabler): without the alias oracle no ternary over memory could be if-converted.** The merge shape is TWO-fold in this IR: a register temp reconciled by a `Copy` in each arm (to_ssa φ-ifies only promoted memory), AND any φ in M (M's only preds are {T,E}, so every φ has arms exactly {T,E}) — both become `Select`. Restricted to NON-FLOAT scalars (backend `csel` is integer; a float merge keeps its branch). `Select(d,ty,c,a,b)` is a new pure/total CORE inst — interp evaluates `canon(ty, c≠0?a:b)`, the backend lowers `cmp;csel` + `ext_r` (mirrors interp's `canon` exactly). `⟦f⟧=⟦if_convert(f)⟧` | **DONE (B4), default-ON.** `opt::{if_convert, speculatable}` + `AliasInfo::fault_free`, runs ONCE after the SSA fixpoint (consumes SSA), before `out_of_ssa`; `dce`+`cfg_simplify` reclaim the dead arms. **Correctness** — the commuting square via `equiv` (`if_convert_semantics`: sel/min/max/abs/mix/nest) + the SAFETY side-condition `if_convert_refuses_unsafe` (a Store/Call/faultable-Load diamond is left untouched). **Improvement** — the `.s` drops a conditional branch per diamond (statically visible); measured in isolation on a two-ternary hot loop: 2 `csel`, cond-branches 3→1, zcc reaches PAR with gcc-O0 (vs the +2% of if_convert-OFF). Gated: torture **1378/0** — the box caught 2 compiler-panic regressions `equiv` was blind to (a φ in M left with no arm for the rewired pred `h` → out_of_ssa panic; fixed by converting M's φs too), each a Side-I fix, not a test edit — csmith confirms |
| LDP/STP PAIRING (B4 — backend; AAPCS64 §5 two-register transfer) | Two ADJACENT single-register memory accesses of the same class `str\|ldr {x\|w\|d\|s}Ra,[base,#oa]` then `…Rb,[base,#ob]` with `ob = oa + sz` and a scaled `imm7`-valid offset ≡ ONE `stp\|ldp Ra,Rb,[base,#oa]` — a Side-II encoding identity (§C6.2 LDP/STP). Guards: same base register (no writeback), no base-register clash, and `ldp` dests must differ. A pure `.s`→`.s` peephole (like `peephole_moves`) — no IR, no semantic reasoning: machine translation-validated by opt-parity | **DONE (B4), default-ON.** `arm64_elf::{parse_ldst, pair_ldst}`, applied per-function-body post-emit after `peephole_moves` (gated by `regalloc`). Proven: `pair_fuses_callee_save_slab`, `pair_fuses_ldp_and_offsets`, `pair_respects_fences` + the callee-save slab visibly pairs (`stp x22,x19` / `stp x20,x21`). Gated: torture 1378/0, opt-parity **1552 PARITY / 0 DIVERGE** |
| SROA (scalar replacement of aggregates — pre-SSA field-split) | a non-escaping aggregate local whose fields never partially overlap is split into independent per-field slots, so `to_ssa` can then promote each field to a register. Every rewritten address is value-identical by the Lea ring identity `⟦Lea(Local(base−c))⟧ = frame−(base−c) = (frame−base)+c = ⟦Add(Lea(Local(base)),c)⟧` ⟹ `⟦f⟧=⟦sroa(f)⟧`. Decomposability gate: every address-tmp use is a Load/Store address or a resolved-const `Add` (no pointer escape), and no two distinct field offsets overlap (unions / cast-punning excluded) | **DONE, default-ON.** `opt::sroa` (runs before `to_ssa`); proven by `equiv` — `sroa_splits_and_commutes` + `sroa_respects_escape_and_overlap` (the gate has teeth) |
| POINTER-IV strength reduction + LFTR (gcc-O1 loop-nest recipe) | a derived address `base + i·stride` recomputed each iteration is replaced by a single MARCHING POINTER advanced by one `add` per trip; linear-function test replacement (LFTR) then retargets the exit test and DROPS the original index φ. `⟦f⟧=⟦pointer_iv(f)⟧` — the marching pointer equals `base + i·stride` at every trip by induction (the ℤ/2ⁿ argument of strength-reduce). PRESSURE-REDUCING (removes a live index + a per-use multiply) ⟹ no `gp_k` cap, unlike licm/SR | **DONE, default-ON.** `opt::pointer_iv`; proven by `pointer_iv_semantics_preserved` (equiv), `pointer_iv_fires_and_stays_finite`, `pointer_iv_declines_scalar_loop` (teeth). Measured: matmul 3.44→1.71×, sieve 2.60→2.10× vs O1, spill-free |
| REMATERIALIZATION (Tier-5 — live-range shortening under pressure) | a PURE, OPERAND-FREE def (`Copy(Imm)` / `Lea` / `FunAddr` / `LabelAddr` — `inst_uses`-empty ⟹ available unconditionally) is recomputed next to its uses instead of held live across a region, trading a spill/reload for one re-evaluation. `⟦f⟧=⟦remat(f)⟧` (recomputing an operand-free pure value is value-identical). Runs LAST (nothing after re-CSEs the clones) | **DONE, PROVEN, ships OFF** — `opt::remat`; proven by `remat_recomputes_pressured_address` + `remat_refuses_operand_bearing_defs` (teeth). Speed-gated like licm/SR on the box A/B — one flag (`ZCC_OPT_ON=remat`) from ON |
| LOOP-INVARIANT CONSTANT HOIST (lever 9 — comparison-immediate hoisting) | a loop-invariant "expensive" immediate (a comparison operand beyond the `cmp` imm12 field) is lifted into ONE preheader `Copy(t, Imm(N))` and the in-loop compare rewritten to read `t` — per-iteration `mov;movk;cmp` becomes a preheader `mov;movk` + a register `cmp`. `⟦f⟧` preserved (reading a temp defined once as `Copy(t,Imm(N))` in a dominating preheader = reading `Imm(N)` in place, a value-numbering identity). PRESSURE-SAFE without a guard (dual of remat: a spilled `t` reloads in one `ldr` ≤ the `mov;movk` it replaces). Comparison operands only — keeps literal `Imm`s visible to the address/arith folds. Runs ABSOLUTE-last (after remat) | **DONE, default-ON.** `opt::hoist_loop_consts`; proven by `hoist_loop_consts_preserves` (equiv). Dominant win on the sieve's linear scans |
...
(The loop passes — licm / strength-reduce / pointer_iv / hoist-const — DO restructure loops, yet stay decidable via explicit fences: single-def gates + induction-on-trip-count, never a Rice-undecidable whole-loop equivalence.)

**The SSA pipeline `opt::optimize_ssa` (the QBE-level projection under CbC) `[main]`:**
`[inline (whole-program, pre-SSA)] ▸ per-function{ sroa ▸ to_ssa ▸ (sccp ∘ const_fold ∘ copy_prop ∘ gvn ∘ cse ∘ load_elim ∘ dce ∘ cfg_simplify [∘ licm ∘ strength_reduce] ∘ pointer_iv)* ▸ if_convert ▸ out_of_ssa ▸ optimize ▸ [remat] ▸ hoist_const }`
(`[·]` = default-OFF, one env flag from ON; everything unbracketed ships ON)
...
backend. Loop transforms now present: `pointer_iv` + `hoist_const` (ON), `licm` +
`strength_reduce` (proven, OFF). Still OMITTED (QBE "most of the win, a fraction of the
complexity"): cross-loop GVN, loop fusion / interchange / unrolling.
