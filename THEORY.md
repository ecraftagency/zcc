...
| Spilling = Belady MIN on a working set (Braun & Hack 2009) | per register class, walk each block forward carrying a WORKING SET `W` of ≤ k values; at the head, live-in non-memory values ordered by next-use distance — **measured along the TRACE, not along the text** (see below), truncated to the budget (what does not fit becomes memory-resident — Belady's provably-optimal rule for a fixed-size cache, which is what a register class is); a use of a value absent from `W` gets a RELOAD into a fresh vreg serving every later use in the block; a call's clobber set counts as registers already spoken for, so what survives a call is ≤ the class's callee-saved count (this SUBSUMES all "crosses a call" reasoning; two values crossing DIFFERENT calls are bound by whichever call comes first). Post-condition: pressure ≤ k at every point — exactly colouring's precondition | `regalloc/spill.rs`. **R0/R1 shipped the sound base case** (spill-at-def, reload-per-use); **R2.2 shipped Braun-Hack proper**, the blocking prerequisite mem2reg forced (the base case collapsed exactly as predicted: sqlite 12,253 → 275,665 frame mem-ops before it). **Two deviations, theory not implementation notes**: (1) **NO SSA reconstruction** — a reload's fresh register is used only inside the block that created it, so its live range is dominated by its definition and SSA holds by construction; a value staying in `W` across an edge keeps its original name. The price is one reload per block-residency rather than one per program region (the §13b spill-traffic excess). (2) **A spilled BLOCK PARAMETER cannot be stored at its definition** — its definition is the block head — so the parameter is removed from the IR and each predecessor writes the slot; one slot per SSA WEB (parameter ∪ arguments), merged only where members do not interfere |
| **The distance Belady ranks by is DYNAMIC, and a back edge runs backwards in reverse postorder** `[2026-08-27]` | Belady's MIN is a theorem about a TRACE: evict the value whose next use is furthest away *in time*. `linear_positions` numbers instructions in reverse postorder, in which a loop's back edge runs BACKWARD — so a value carried around a loop is read at a LOWER position than the latch that passes it on, the static query finds nothing after the latch, and the answer is `usize::MAX`: "never used again", the strongest possible reason to evict. The values a loop uses most are therefore ranked as dead. **Measured on `tests/bench/nestjoin.c`: the loop index, the loop pointer and the accumulator were all spilled out of a four-million-iteration inner loop while twenty-four values used only after both loops kept their registers — 8 ms against gcc -O1's 1 ms.** The distance is therefore computed by walking out through the loop nest: a use still ahead this iteration is its static distance; a use behind but inside the same loop is one wrap; neither, and the remaining trips are the cost of leaving, asked again of the enclosing loop. It is also measured over the SSA WEB rather than the value, because mem2reg splits one C variable into a chain whose every link has exactly one use — being passed to the next — so cold and hot values report the same distance at the very edge where the choice is made, and because a web is the granularity at which eviction is PAID | `regalloc/spill.rs::Trace`, proof `eviction_ranks_by_dynamic_distance_not_text_order`; the trip-count constant is `MEASURED M12` |
...
| **A LOCATION IS A REGISTER *OR* A SPILL SLOT** (the edge's parallel copy is not a register-only object) | the row above is stated over registers, and a spilled block parameter puts a SLOT on the same edge: the parameter leaves the IR and every predecessor writes its slot (the A7 deviation (2) above). Read-before-write — the defining property of a parallel copy — must then hold across the register/slot boundary too. When an argument the edge carries is itself resident in a slot the edge overwrites, its read is materialized into a fresh value BEFORE the writes run; that is the same move the windmill makes for a register cycle, with a slot as the location. **Measured, 2026-08-27:** without it, a rotation of three pointers around a loop back edge emitted `str x13,[sp,#88]` and then `ldr x13,[sp,#88]`, so the rotation lost a pointer — sqlite's `wherePathSolver` chooses its join order with exactly that rotation, and the zcc-built sqlite CLI SIGSEGV'd on any two-table query while the full gate, including a 20,000-program generated seal, stayed green | `regalloc/spill.rs::evict_params`, proof `an_edge_reads_a_slot_before_the_edge_overwrites_it` + the generated family `every_pointer_rotation_under_pressure_keeps_its_permutation` |
| Correctness = renaming bisimulation | allocation renames values and may route some through memory; it must not change what the function computes. Proven as `⟦mir_v⟧ = ⟦mir_p⟧` under one interpreter, plus the structural checks: no vreg survives, every fixed operand satisfied, every reload preceded by a spill of its slot ON EVERY PATH, no parallel copy left. **Those structural checks run on EVERY COMPILE** (`compile::allocated`), not only on test fixtures — a theorem the shipping pipeline never asks is not a theorem the compiler has. They are asked post-allocation and PRE-frame-lowering, the earliest layer where they are decidable: the reload obligation is stated in the `Spill`/`Reload` vocabulary and `ldst_pair` spends it, folding two spills into one `stp` that carries an address rather than a slot number | `regalloc/verify.rs`, `compile.rs`, `regalloc/tests.rs` |
