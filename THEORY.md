...
| BACKEND PEEPHOLE (Phase C — redundant register-move elimination) | the emitter is an x0-accumulator machine (every value flows through x0 then to/from its home register), so a value is stored to its home and immediately reloaded: `mov xH,x0 ; mov x0,xH`. Track a per-register 64-bit value-equivalence within a straight-line region; DROP a `mov xD,xS` when the model already proves D≡S (a verified no-op). Soundness: every recognized DEF gives its dst a FRESH value-id; any branch/call/label/unknown-mnemonic FLUSHES the model ⟹ no stale ≡ ever survives a value change or crosses control flow. Machine-level translation-validation, not IR `equiv` (post-codegen text) | **DONE (Phase C), default-ON — the BIGGEST MEASURED WIN.** `arm64_elf::peephole_moves` (one-file-per-target law: machine peephole lives in the target file), toggle `ZCC_OPT_OFF=peephole`. Measured: matmul 398→335 insns (240→177 movs); **bench geomean 1.39×→1.07× vs gcc-O0, BEATING gcc-O0 on loops (0.75×) and sieve (0.63×)**. Gated: 5 machine unit tests (`peephole_drops_redundant_roundtrip`/`…_keeps_move_after_clobber`/`…_flushes_at_label`/`…_flushes_on_unknown`/`…_preserves_distinct_move`) + torture 0 FAIL + opt-parity 0 DIVERGE. Only a MEASURED win defaults ON — this is one |
