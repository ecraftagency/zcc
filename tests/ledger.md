...
| R2 | 2026-08-18 | redis crash-report/stacktrace: `integration/logging` ×7 + `moduleapi/crash` ×2 — server zcc không sinh stack trace/crash report khi crash; gcc referee pass → án zcc (nghi backtrace()/unwind/symbol, CHƯA điều tra) | `cd /build/redis && ./runtest --single integration/logging` (box) | zcc | MỞ | `b0-redis-20260818-r2/b0.log` |
| R3 | 2026-08-18 | redis `unit/other` "Process title set as expected" — setproctitle không ăn trên binary zcc (nghi argv/environ layout, CHƯA điều tra) | `cd /build/redis && ./runtest --single unit/other` (box) | zcc | MỞ | `b0-redis-20260818-r2/b0.log` |
