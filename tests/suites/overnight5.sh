#!/bin/sh
# Vòng đóng sổ 18/8 tối (overnight5) — seed mang TRỌN đợt fix trong ngày:
...
LOG=/suites/overnight5-20260818
B=/build
mkdir -p "$LOG" "$B"
...
sq_bench() {
    s=$(perl -MTime::HiRes=time -e "print int(time()*1000)"); "$ZCC" -c /suites/sqlite/sqlite3.c -o /tmp/sqb.o; e=$(perl -MTime::HiRes=time -e "print int(time()*1000)")
    echo "sqlite3.c 262k dòng: $(( e - s )) ms"
}
