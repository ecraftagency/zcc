#!/bin/sh
# Vòng đóng sổ 18/8 chiều — sau fix $-in-#if0 (sqlite testfixture) + transparent_union (glibc bind/connect dưới _GNU_SOURCE — redis/nginx EFAULT). Binary seed đã mang cả 2 fix.
...
LOG=/suites/overnight4-20260818
B=/build
mkdir -p "$LOG" "$B"
...
sq_bench() {
    s=$(date +%s%N); "$ZCC" -c /suites/sqlite/sqlite3.c -o /tmp/sqb.o; e=$(date +%s%N)
    echo "sqlite3.c 262k dòng: $(( (e-s)/1000000 )) ms"
}
