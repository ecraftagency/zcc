#!/bin/sh
# Vòng đóng sổ 18/8 tối (overnight6 — overnight5 hỏng 2 mục vì harness: bench var đè run(), git rm trash; seed thêm stack 256MB driver trị segv testfixture) — seed mang TRỌN đợt fix trong ngày:
# transparent_union, $-ident, C99-đủ (digraphs + sizeof(VLA) runtime, trọng tài
# run.sh nâng -std=c99), __sync_fetch_and_and/or/xor, _Static_assert, và
# CHAR UNSIGNED trên ELF (lexer '\377'=255 — nghi phạm chính của cụm git
# blame/merge + torture mới). Án tồn từ overnight4: torture 9 case pr*,
# git 4 cụm (t7600/t8002/t8012/t7112) + t9350/t9902, redis unixsocket
# "Out of memory" (redis-cli OOM path), sqlite-tcl segv (đã biến mất với
# binary mới khi tái thẩm tay — theo dõi tại đây).
...
LOG=/suites/overnight6-20260818
B=/build
mkdir -p "$LOG" "$B"
...
# bench tốc độ compile amalgamation (Vu 18/8: theo dõi mỗi vòng)
sq_bench() {
    sb=$(perl -MTime::HiRes=time -e "print int(time()*1000)"); "$ZCC" -c /suites/sqlite/sqlite3.c -o /tmp/sqb.o; eb=$(perl -MTime::HiRes=time -e "print int(time()*1000)")
    echo "sqlite3.c 262k dòng: $(( eb - sb )) ms"
}
run sqlite-bench sq_bench
...
git_build() {
    set -e
    chmod -R u+rwX "$B/git" 2>/dev/null || true # trash t5570 của runner làm rm chết (VirtioFS)
...
# differential per-file cụm nghi phạm merge/blame: zcc-git vs gcc-git cùng box
git_ref() {
    set -e
    rm -rf "$B/git-gcc" && cp -r /suites/git "$B/git-gcc" && cd "$B/git-gcc"
    make -j"$NP" NO_RUST=1 NO_GETTEXT=1 NO_TCLTK=1 NO_CURL=1 NO_EXPAT=1 \
        FSMONITOR_DAEMON_BACKEND= FSMONITOR_OS_SETTINGS= >/dev/null 2>&1
    chown -R runner "$B/git-gcc"
    for tt in t7600-merge t8002-blame t8012-blame-colors t7112-reset-submodule; do
        su runner -c "cd '$B/git-gcc/t' && ./$tt.sh" > "$LOG/git-ref-$tt.log" 2>&1 || true
        su runner -c "cd '$B/git/t' && ./$tt.sh -v" > "$LOG/git-zcc-$tt.log" 2>&1 || true
        zf=$(grep -c '^not ok' "$LOG/git-zcc-$tt.log" || true)
        rf=$(grep -c '^not ok' "$LOG/git-ref-$tt.log" || true)
        echo "$tt: zcc-fail=$zf ref-fail=$rf" >> "$LOG/git-diff-verdict.txt"
    done
}
if ! grep -q "git-t-full: PASS" "$LOG/SUMMARY.txt"; then run git-referee git_ref; fi
