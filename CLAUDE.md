...
- **One module per target** under `src/codegen/`; `codegen/mod.rs` is the single door (`emit(&Ast)->String`). ABI / section / asm syntax live entirely in the target module. `src/codegen/` is **ELF-only** (AArch64 Linux; x86_64 deferred; macOS is the clang oracle only). A target grown past readability becomes a **directory module split along the Law-1 seams** (Side-II encoding · Side-I lower/emit/peephole), proven byte-identical (Article E) — map in `src/codegen/arm64_elf.md`.
