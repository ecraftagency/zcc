...
- **M14 — redis (cúp giai đoạn 2, ĐẠT 2026-08-17)**: `make CC=zcc MALLOC=libc` — server + cli + benchmark link, vendored deps (lua, hiredis, hdr_histogram, fpconv, xxhash, tre, linenoise) đều compile bằng zcc. **Gate `tests/m14.sh`: clone sạch → build → `redis-server` chạy, `redis-cli` PING→PONG + SET/GET/INCR đúng.** Quyết định hệ trọng trong đường tới gate:
  ...
  - **`__atomic_*` = macro đổ về `__sync_*`** (bảng ATOMIC_MACROS ext.rs) — load = fetch_add(p,0), CAS qua statement-expr + `__typeof__`; memorder bỏ qua (luôn seq_cst). fetch_or/and/xor chỉ jemalloc đòi — chưa làm (MALLOC=libc né).
  - **gnu89 inline nghĩa là gì với zcc**: định nghĩa inline không có declaration trần → phát `.weak_definition` (non-static) + **DCE per-TU** (không ai với tới thì không phát — như clang; bắt buộc vì body inline trong server.h tham chiếu symbol mà redis-cli không link). Có declaration trần → external thật (C99 6.7.4p7, logreqres.c dựa vào). Kéo theo `.subsections_via_symbols` (không có thì ld không coalesce weak).
  ...
  - `__thread` = no-op storage class (io-threads=1 nên main thread sở hữu; TLS Mach-O thật `@TLVP` là NỢ nếu ai đòi multi-io-thread). Builtin bit-manip (`__builtin_bswap*/clz*/ctz*/popcount*`) = macro statement-expr thuần trong ext.rs BIT_MACROS, không cần codegen. Computed include `#include MACRO` (C89 3.8.2 chuẩn, không phải ext). String init bọc ngoặc `char x[]={ "s" }` (C89 3.5.7). Splice `\`+newline TRONG string literal. Driver thêm `-shared`/`-dynamiclib` → `ld -dylib`. `typedef enum : type` (SDK malloc.h). Local shadow typedef (`quicklist *quicklist`) — lookup typedef fail khi local trùng tên và `in_fn`. Predefine `__ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ 110000` (macOS 11 = OS Apple Silicon đầu tiên; thiếu nó redis config.h rơi vào nhánh `struct stat64` ẩn). Stub xóa thêm: dlfcn.h, stdint.h, limits.h (SDK thật phục vụ — `uint_fast64_t`, `Dl_info`, `IOV_MAX`). Stub thêm: INFINITY/NAN/M_PI/fpclassify (math.h), `_IO*`+setvbuf (stdio.h), `getprogname` (stdlib.h — thiếu là implicit-int CẮT POINTER 32-bit, crash strdup trong spt_init, đúng bẫy ASLR/implicit-int đã ghi từ M8).
  - Bẫy build lặp lại 2 lần: `.o`/`.a` cũ build bằng zcc thế hệ trước lẫn trong cây redis → lỗi link ma quái ("duplicate", "undefined" không giải thích được). Debug lỗi link redis: LUÔN `make distclean` + `rm src/*.o` trước.
...
- **Nợ tồn giai đoạn 2 (trả dần trong M15/M16, đã có harness sẵn):**
  - ~~nginx-tests rerun~~ TRẢ XONG 2026-08-17: minimal 493 file/1136 test All PASS; build FULL (ssl+http2+stream+mail, pcre2+openssl@3) 5346 test, fail-chỉ-zcc = RỖNG (trọng tài nginx-cc cùng config fail giống hệt — chi tiết tests/README.md).
...
- **Gate khoa học (vét cạn không gian hữu hạn — chạy khi đụng codegen/ABI/const-eval):**
  ...
  - `tests/cpp.sh` (+`gen_cpp.py`) — lớp preprocessor như hệ viết lại hạng: ma trận tương tác cơ chế expansion (prescan/paste/stringize/sơn-xanh/rescan; dạng chuẩn reify qua `#` thành string runtime — KHÔNG diff `-E` vì format không được spec đặc tả) + vét cạn số học `#if` (long/ulong 3.8.1, oracle kép zcc==cc==python). Lý thuyết + exclusion list undefined/unspecified: đọc đầu gen_cpp.py. Bắt ngay bug evaluator #if bỏ dấu unsigned lần chạy đầu.
  - Sổ kiểm kê TOÀN BỘ tài sản test (bản đồ 6 lớp proof, suite ngoài + cách dựng lại, bẫy đã trả học phí): `tests/README.md`.
